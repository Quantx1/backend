...
Resolves the model (registry-first, disk fallback), builds the momentum feature
...
ens — Phase 2), the latest weekly-cached values are LEFT-merged per symbol via
``ml.features.forecast_serving.latest_forecasts``; an absent cache degrades
(forecast cols NaN, loud log) instead of erroring. Honest-empty if the model
...
class MomentumEngine:
    ...
    def _merge_forecasts(self, feats: pd.DataFrame) -> pd.DataFrame:
        ...
        (tsfm/kronos/ens for momentum); serving builds only price/RS features,
        so without this merge ``dropna`` on the artifact contract would kill
        every row. The latest cached forecast per symbol applies to the
        current scoring bar — staleness up to stride + weekly-refresh cadence
        is by design (Phase 2 two-speed inference). Missing cache: the columns
        are added as NaN so the artifact contract is satisfied structurally
        (LightGBM tolerates NaN at predict), with a loud degraded=True log.
        """
        ...
        fx = None
        try:
            fx = latest_forecasts("momentum")
        except Exception as exc:  # noqa: BLE001 — cache read must never kill scoring
            logger.warning("momentum forecast-cache read failed: %s", exc)
        if fx is None:
            self.forecast_degraded = True
            logger.warning(
                "momentum: forecast cache absent — scoring degraded=True "
            ...
            logger.info(
                "momentum: merged %d forecast cols for %d symbols (age=%dd)",
    ...
    def run(self, top_n: int = 20, universe_limit: Optional[int] = None) -> List[MomentumSignal]:
        ...
        try:
            ...
            logger.info("momentum benchmark (NIFTY) load failed: %s", exc)
            bench = None
        try:
            feats_all = build_momentum_features(panel[cols], benchmark=bench)
        except Exception as exc:
            logger.warning("momentum feature build failed: %s", exc)
            self.status = "no_data"
            return []
        ...
        try:
            ...
            logger.warning("momentum feature frame vs artifact contract failed: %s", exc)
            self.status = "no_data"
            return []
        if feats_all.empty:
            self.status = "no_data"
